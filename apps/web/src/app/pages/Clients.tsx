...
export default function Clients() {
  ...
  const [form, setForm] = useState({ name: '', city: '', address: '', contact: '', phone: '', email: '', latitude: '', longitude: '', salesOwnerId: '' });
  ...
  async function submit(event: React.FormEvent) {
    ...
    setForm({ name: '', city: '', address: '', contact: '', phone: '', email: '', latitude: '', longitude: '', salesOwnerId: '' });
    await load();
  }
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <Card className="bg-zinc-900/50 border-zinc-800">
        <CardHeader><CardTitle className="text-white">Novo Cliente</CardTitle></CardHeader>
        <CardContent>
          <form onSubmit={submit} className="grid md:grid-cols-3 gap-3">
            ...
            <Button className="bg-blue-500 hover:bg-blue-600 md:col-span-3"><Plus className="h-4 w-4 mr-2" />Adicionar Cliente</Button>
          </form>
        </CardContent>
      </Card>
        ...
        {clients.map((client) => (
          <Card key={client.id} className="bg-zinc-900/50 border-zinc-800">
            <CardContent className="p-5 space-y-3">
              ...
              <p className="text-sm text-zinc-400 flex items-center gap-2"><MapPin className="h-4 w-4" />{client.address} · {client.city}</p>
              {client.phone && <p className="text-sm text-zinc-400 flex items-center gap-2"><Phone className="h-4 w-4" />{client.phone}</p>}
              {client.email && <p className="text-sm text-zinc-400 flex items-center gap-2"><Mail className="h-4 w-4" />{client.email}</p>}
